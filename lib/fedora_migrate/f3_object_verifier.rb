  ...
  class ObjectMover < Mover
    ...
    def target
      ...
      if @options[:target_constructor] == 'elections'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source,'RECORD-XML', nil, 'ebeck01', @keep_extent).build
      elsif @options[:target_constructor] == 'eads'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source, 'Archival.xml', nil, utln, @keep_extent).build
      elsif @options[:target_constructor] == 'images'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source, 'Archival.tif', nil, utln, @keep_extent).build
      elsif @options[:target_constructor] == 'pdfs'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source, 'Archival.pdf', 'Transfer.binary', utln, @keep_extent).build
      elsif @options[:target_constructor] == 'teis'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source, 'Archival.xml', nil, utln, @keep_extent).build
      elsif @options[:target_constructor] == 'audio'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source, 'ARCHIVAL_WAV', 'ARCHIVAL_XML', utln, @keep_extent).build
      elsif @options[:target_constructor] == 'video'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source, 'Archival.video', 'ARCHIVAL_XML', utln, @keep_extent).build
      elsif @options[:target_constructor] == 'rcrs'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source, 'RCR-CONTENT', nil, utln, @keep_extent).build
      elsif @options[:target_constructor] == 'generics'
        @target ||= FedoraMigrate::TuftsSourceVerifier.new(source, 'GENERIC-CONTENT', nil, utln, @keep_extent).build
      else
